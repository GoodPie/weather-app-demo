...
defineOptions({ name: 'VoltPopover' });
...
defineExpose({
    toggle: (event: Event, target?: HTMLElement) => (el.value as any)?.toggle?.(event, target),
    show: (event: Event, target?: HTMLElement) => (el.value as any)?.show?.(event, target),
    hide: () => (el.value as any)?.hide?.()
});
</script>
